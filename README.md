...
都市の天気検索・お気に入り・5日間予報を提供する天気アプリです。
認証・管理者ページ・PDF 出力など 業務アプリ要素をフルスタックで実装 しています。
...
## 🔧 技術的な工夫ポイント（要点）
- OpenWeather API の無料枠制限を考慮し、Prisma を使ったキャッシュ機構を実装
- 都市検索は LIKE 検索 + 読み仮名対応で、曖昧検索を実現
- Next.js App Router と Route Handlers を採用し、SSR＋APIをシンプルに構成
- 認証はトークン方式で、メール認証・パスワード再設定を安全に実装
- 管理者向けに CSV 出力・検索・並び替えを実装し、業務アプリとしての要件を再現
